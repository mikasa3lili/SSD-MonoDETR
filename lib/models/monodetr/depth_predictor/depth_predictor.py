...
class DepthPredictor(nn.Module):
    ...
    def forward(self, feature, mask, pos):
        ...
        src1 = self.depth_head(src)
        depth_logits = self.depth_classifier(src1)
        ...
        depth_embed = self.depth_encoder(src2, mask, pos)
        depth_embed = depth_embed.permute(1, 2, 0).reshape(B, C, H, W)

        depth_pos_embed_ip = self.interpolate_depth_embed(weighted_depth)
        depth_embed2 = depth_embed + depth_pos_embed_ip

        return depth_logits, depth_embed2, weighted_depth, depth_embed

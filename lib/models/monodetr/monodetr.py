...
class MonoDETR(nn.Module):
    ...
    def __init__(self, backbone, depthaware_transformer, depth_predictor, num_classes, num_queries, num_feature_levels,
        ...
        self.myconv = torch.nn.Conv2d(512, 512, 1)
        self.mylinear = torch.nn.Linear(80*24, 50)
    ...
    def forward(self, images, calibs, targets, img_sizes):
        ...
        pred_depth_map_logits, depth_pos_embed, weighted_depth, depth_map = self.depth_predictor(srcs, masks[1], pos[1])
        hs,      init_reference, inter_references, inter_references_dim, inter_my_weight, enc_outputs_class, enc_outputs_coord_unact \
            = self.depthaware_transformer(srcs, depth_map, masks, pos, query_embeds, depth_pos_embed)
        ...
        for lvl in range(hs.shape[0]):
            ...
            depth_map = F.grid_sample(
                weighted_depth.unsqueeze(1), #(16,24,80)
                outputs_center3d, #(16,50,1,2)
                mode='bilinear',
        ...
        out['pred_my_weight'] = inter_my_weight[-1]
...
class SetCriterion(nn.Module):
    ...
    def loss_wsm(self, outputs, targets, indices, num_boxes):
        idx = self._get_src_permutation_idx(indices)
        src_my = outputs['pred_my_weight'][idx]
        target_depths = torch.cat([t['depth'][i] for t, (_, i) in zip(targets, indices)], dim=0).squeeze()
        gt_boxes2d = torch.cat([t['boxes'] for t in targets], dim=0) * torch.tensor([80, 24, 80, 24], device='cuda')
        gt_w = gt_boxes2d[:,2]

        fake_depth = src_my[:, 0] * 1 + src_my[:, 1] * 3 + src_my[:, 2] * 5 + src_my[:, 3] * 7 + src_my[:, 4] * 9
        loss_w = torch.abs((fake_depth-gt_w))

        target_sort, t_id = torch.sort(target_depths)
        fake_sort, f_id = torch.sort(fake_depth, descending=True)
        # rbo_score = self.rbo_score(t_id.cpu().numpy().tolist(), f_id.cpu().numpy().tolist(), 0.5)
        fl = f_id.cpu().numpy().tolist()
        tl = t_id.cpu().numpy().tolist()
        dl = []
        for i, v in enumerate(tl):
            i_in_fl = fl.index(v)
            d = abs(i - i_in_fl)
            dl.append(d)
        pos_weight = torch.log10(torch.Tensor(dl)+1)
        weight_loss_w = loss_w * pos_weight.cuda()
        sum = torch.sum(weight_loss_w)
        losses = {}
        wsm = sum / num_boxes
        losses['loss_wsm'] = wsm
        return losses

    def rbo_score(self, l1, l2, p):
        s1 = set()
        s2 = set()
        max_depth = len(l1)
        score = 0.0
...
def build(cfg):
    ...
    weight_dict['loss_wsm'] = cfg['wsm_loss_coef']
    ...
    losses = ['labels', 'boxes', 'cardinality', 'depths', 'dims', 'angles', 'center', 'depth_map','wsm']

...
class DepthAwareTransformer(nn.Module):
    def __init__(
        ...
        decoder_layer = DepthAwareDecoderLayer(d_model, dim_feedforward, dropout, activation, num_feature_levels, nhead, dec_n_points)
    ...
    def interpolate_img_by_xy_d(self, img, xy):
        # (B,C,H,W)
        xy1 = xy.detach().cpu().numpy().copy()
        normal_shape = np.array([80., 24.])
        xy1[:, :, 0] *= 80.
        xy1[:, :, 1] *= 24.
        xy11 = torch.from_numpy(np.float32(xy1 * 2 / (normal_shape - 1.) - 1.)).unsqueeze(1).cuda()
        ret_img = grid_sample(img, xy11, padding_mode='zeros', mode='bilinear')
        ret_img = ret_img.squeeze(2).permute(0, 2, 1)
        return ret_img

    def forward(self, srcs, depth_map, masks, pos_embeds, query_embed=None, depth_pos_embed=None):
        ...
        # decoder
        hs, inter_references, inter_references_dim, inter_my_weight = self.decoder(
            tgt,
            reference_points,
            memory,
            srcs,
        ...
        inter_outmy_weight = inter_my_weight
        if self.two_stage:
            return hs, init_reference_out, inter_references_out, inter_references_out_dim, enc_outputs_class, enc_outputs_coord_unact
        return hs, init_reference_out, inter_references_out, inter_references_out_dim, inter_outmy_weight, None, None
...
class VisualEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        # self attention
        src2, my_weight = self.self_attn(self.with_pos_embed(src, pos), reference_points, src, src, src, spatial_shapes, level_start_index, padding_mask)
...
class VisualEncoder(nn.Module):
    ...
    @staticmethod
    def get_reference_points(spatial_shapes, valid_ratios, device):#s_s:[4,2]:tensor([[48,160],[24,80],[12,40],[6,20]]) v_r:(16,4,2) all 1
        ...
        reference_points = reference_points[:, :, None] * valid_ratios[:, None] #(16,10200,4,2)
...
class DepthAwareDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        tgt2, my_weight = self.cross_attn(self.with_pos_embed(tgt, query_pos),
                               reference_points,src, s1, depth_map, src_spatial_shapes, level_start_index, src_padding_mask)
...
class DepthAwareDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, s1, depth_map, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.return_intermediate:
                ...
                if my_weight is None:
                    intermediate_my_weight.append(torch.ones(output.shape[0], 50, 5).cuda())
                else:
                    intermediate_my_weight.append(my_weight)

        if self.return_intermediate:
            return torch.stack(intermediate), torch.stack(intermediate_reference_points), \
                   torch.stack(intermediate_reference_dims), torch.stack(intermediate_my_weight)

        return output, reference_points, my_weight
